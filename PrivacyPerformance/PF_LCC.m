function [S_LCC,S_tilde] = PF_LCC(I0,TotalTime,vehicle_index,PerturbedType)
    ...
    brake_amp           = 7;   % brake amplitude of brake perturbation                   
    % ------------------------------------------
        ...
        if PF_enable
            ...
            for i=1:(m+n+2)
                if i == PF_index
                    ...
                    PF{i} = PF{i}.MonteCarlo(NumStep,PerturbedType);
                    PF{i} = PF{i}.Randomizer(I0);
                    disp(PF{i}.transfer_kernel(1))
                else
                    PF{i} = 0;
                end
            end
        ...
        for k = 1:NumStep - 1
            ...
            switch PerturbedType
                ...
                case 2     % braking
                    ...
                    if k*Tstep < brake_amp/2 && k*Tstep >= 0
                        percent = 1 - k*Tstep/brake_amp/2;
                        S(k,PerturbedID+1,3) = -brake_amp*percent;
                    elseif k*Tstep < (brake_amp/2)*2 && k*Tstep >= brake_amp/2
                        percent = 1 - (k*Tstep-brake_amp/2)/brake_amp;
                        S(k,PerturbedID+1,3) = brake_amp*percent;
                    else
                        S(k,PerturbedID+1,3) = 3*(rand(1)-0.5);
                    end
