function [S_LCC,S_tilde] = PF_LCC_scenario3(I0_preceding, I0_following ,TotalTime, vehicle_index_preceding)
    ...
    for PF_enable = 0:1 
        ...
        % ---------------------------------------------------------
        PF_index = vehicle_index_preceding;
        if PF_enable
            N_tilde_kappa = 10;
            PF = cell(1,n);
            for i=1:(m+n+2)
                if i == PF_index
                    ...
                    PF{i} = PF{i}.Randomizer(I0_preceding);
                elseif i == (PF_index + 1)
                    PF{i} = PrivacyFilter(1,N_tilde_kappa);
                    PF{i} = PF{i}.Randomizer(I0_following);
                else
                    PF{i} = 0;
                end
            end
            ...
            switch PerturbedType
                ...
                case 2     % braking
                    if PerturbedID == 0
                        ...
                        if (k*Tstep>21)&&(k*Tstep<21.5)
                            S(k,PerturbedID+1,3) = 5;
                        end
    
                    else
                        if (k*Tstep>5)&&(k*Tstep<6)
                            S(k,PerturbedID+1,3) = 5;
                        end
