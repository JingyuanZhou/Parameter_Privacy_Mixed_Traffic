...
for k = 1:T-1
    ...
    if k == 1
        ...
        yd_ori(:,k) = measure_mixed_traffic(S(k,2:end,2),S(k,:,1),ID,v_star,s_star,measure_type);
    else
        if PF_enable
            [tilde_vel_cur,tilde_spacing_cur] = pseudo_state_generator(S(k,2:end,2),S(k,:,1),S(k-1,2:end,2),S(k-1,:,1),PF,ID);
            yd(:,k) = [(tilde_vel_cur-v_star)';(tilde_spacing_cur-s_star)'];
            yd_ori(:,k) = measure_mixed_traffic(S(k,2:end,2),S(k,:,1),ID,v_star,s_star,measure_type);
        else
            yd(:,k) = measure_mixed_traffic(S(k,2:end,2),S(k,:,1),ID,v_star,s_star,measure_type);
        end
        %yd(:,k) = [(tilde_vel_cur-v_star)';(tilde_spacing_cur(pos_cav)-s_star)'];
    end
end
k = k+1;
if PF_enable
...
if PF_enable
    save(['_data\trajectory_data_collection\data',num2str(data_str),'_',num2str(i_data),'_noiseLevel_',num2str(acel_noise),'_PfEnable_','.mat'],...
        'hdv_type','acel_noise','Up','Yp','Uf','Yf','Ep','Ef','T','Tini','N','ID','Tstep','v_star');
    Y   = hankel_matrix(yd_ori,Tini+N);
    Yp  = Y(1:Tini*p_ctr,:);
    Yf  = Y((Tini*p_ctr+1):end,:);
    save(['_data\trajectory_data_collection\data',num2str(data_str),'_',num2str(i_data),'_noiseLevel_',num2str(acel_noise),'_PfDisable_','.mat'],...
        'hdv_type','acel_noise','Up','Yp','Uf','Yf','Ep','Ef','T','Tini','N','ID','Tstep','v_star');
